...
import (
	...
	"github.com/surfaceyu/namelo/pipeline"
	"github.com/surfaceyu/namelo/scheduler"
...
func (mgr *RoomManager) Join(s *session.Session, msg []byte) error {
	...
	if !found {
		room = &Room{
			group: namelo.NewGroup(fmt.Sprintf("room-%d", testRoomID)),
		}
...
func main() {
	...
	namelo.Listen(":3250",
		...
		namelo.WithPipeline(pip),
		namelo.WithCheckOriginFunc(func(_ *http.Request) bool { return true }),
		namelo.WithWSPath("/nano"),
		namelo.WithDebugMode(),
		namelo.WithSerializer(json.NewSerializer()), // override default serializer
		namelo.WithComponents(components),
	)
}
